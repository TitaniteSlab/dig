...
terra.CollisionMap = ig.Map.extend({
	...
	traceFixed: function( x, y, vx, vy, objectWidth, objectHeight, returnFirst ) {
		x = x - this.pos.x;
		y = y - this.pos.y;
		return this.trace( x, y, vx, vy, objectWidth, objectHeight, returnFirst )
	},
	
	trace: function( x, y, vx, vy, objectWidth, objectHeight, returnFirst ) {
		x = x - this.pos.x;
		y = y - this.pos.y;
		...
		var res = {
			...
			// Hacky default
			tileIndex: {
				x: Math.floor((this.pos.x + x) / this.tilesize), 
				y: Math.floor((this.pos.y + y) / this.tilesize)
			}
		};
		...
		if( steps > 1 ) {
			...
			for( var i = 0; i < steps && (sx || sy); i++ ) {
				this._traceStep( res, x, y, sx, sy, objectWidth, objectHeight, vx, vy, i, returnFirst );
		...
		// Just one step
			...
			this._traceStep( res, x, y, vx, vy, objectWidth, objectHeight, vx, vy, 0, returnFirst );
		}
		
		res.pos.x = res.pos.x + this.pos.x;
		res.pos.y = res.pos.y + this.pos.y;
		return res;
	},
	
	
	_traceStep: function( res, x, y, vx, vy, width, height, rvx, rvy, step, returnFirst ) {
				...
				for( var tileY = firstTileY; tileY < lastTileY; tileY++ ) {
					if( prevTileX != -1 ) {
						...
						if(	
							...
							this._checkTileDef(res, t, x, y, rvx, rvy, width, height, prevTileX, tileY, returnFirst) 
						) {
							break;
						}
					}
					...
					if(
						...
						(t > 1 && this._checkTileDef(res, t, x, y, rvx, rvy, width, height, tileX, tileY, returnFirst)) // slope?
					) {
						res.tileIndex = {x: tileX, y: tileY};
			...
			if( tileY >= 0 && tileY < this.height ) {
				for( var tileX = firstTileX; tileX < lastTileX; tileX++ ) {
					if( prevTileY != -1 ) {
						...
						if( 
							t > 1 && t <= this.lastSlope &&
							this._checkTileDef(res, t, x, y, rvx, rvy, width, height, tileX, prevTileY, returnFirst) ) {
							break;
						}
					}
	...
	_checkTileDef: function( res, t, x, y, vx, vy, width, height, tileX, tileY, returnFirst ) {
