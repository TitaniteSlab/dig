...
ig.BackgroundMap = ig.Map.extend({	
	...
	preRenderChunk: function( cx, cy, w, h ) {
		var tw = w / this.tilesize / ig.system.scale,
			th = h / this.tilesize / ig.system.scale;
	...
	drawPreRendered: function() {
		...
		for( var cy = minChunkY; cy < maxChunkY; cy++ ) {
			...
			for( var cx = minChunkX; cx < maxChunkX; cx++ ) {
				...
				if (chunk != null) {
					...
					//console.log('drawing chunk ' + cx + ',' + cy + ' at ' + x + ',' + y);
		...
		//console.log('num chunks drawn: ' + numDrawn);
		//console.log('out of ' + this.preRenderedChunks[0].length + ' x ' + this.preRenderedChunks.length);
	},
			...
			for( var mapX = -1, pxX = pxMinX; pxX < pxMaxX; mapX++, pxX += this.tilesize ) {
				...
				if( (tile = this.data[tileY][tileX]) ) {
					if( (anim = this.anims[tile-1]) ) { 
						...
						this.tiles.drawTile( this.offset.x + pxX, this.offset.y + pxY, tile-1, this.tilesize );
